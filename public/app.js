...
// let sourceLanguage = 'ko'; // 기본값으로 ko
    ...
    debouncer = setTimeout(() => {
        ...
        xhr.onreadystatechange = () =>{
            if(xhr.readyState ===4 && xhr.status ===200){
                //최종적으로 papago가 번역해준 번역된 텍스트 결과를 받는 부분(추후 작성) + 서버의 응답 경과를 확인하는 부분
        ...
        const requestData = {
            text,//text: text, 프로퍼티와 변수명이 동일할 경우, 하나만 작성해도 됨.
            targetLanguage, // targetLanguage: targetLanguage 와 같음

        };

        //클라이언트가 서버에게 보내는 요청 데이터의 형식이 json 형식임을 명시
        xhr.setRequestHeader('Content-type', 'application/json'); //text/html 등 여러가지가 있음
        //위 내용에서 header: 제품의 설명서라고 보면 된다.

        //보내기 전에 해야 할 일: JS object를 json으로 변환(직렬화)
        const jsonData = JSON.stringify(requestData); // requestData를 직렬화하여 jsonData에 저장
